...
/*   Updated: 2025/05/15 11:51:09 by hfegrach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (lexer->c)
	{
		...
		if (lexer->c == '#') // should advance all next characters without saving it as a token
			return (lexer_advance_with(lexer,
					lexer_skip_comment(lexer))); // eroooooooooooooooooooooooooooooooooooor
		...
		if (lexer->i == lexer->line_size - 1)
			return (init_token(ft_strdup("newline"), EOI));// return NULL;
	}
	return (NULL);
}
